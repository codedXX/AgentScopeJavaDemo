...
**会话管理：** PostgreSQL 的 `chat_session` 和 `chat_turn` 保存完整会话与成功的问答轮次；MyBatis-Plus 负责读写，Flyway 负责建表。每轮从数据库读取最近 10 轮完整问答，新建 AgentScope Agent、Memory 和工具记录器，重启后仍可追问；服务不缓存会话状态。完整问答在一次数据库事务中保存；数据库写入失败时本轮返回错误。同一会话若从多个客户端同时提问，两轮可能读到相同的旧历史，回答写入顺序取决于完成时间。
